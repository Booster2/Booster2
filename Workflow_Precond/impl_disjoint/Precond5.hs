		...
		loc_cond = if delayed gi1 && delayed gi2                   		then Atom "itrue" else	
			   ...
			   if lut2!par1 == Atom "itrue" || lut2!par2 == Atom "itrue"	  then 
				if lut2!par1 == Atom "itrue" && lut2!par2 == Atom "itrue" then Atom "itrue" else		-- gi1 --> wp(ai1,T) /\ g2 --> wp(ai2,T) 
                                ...
                                if lut2!par1 == Atom "itrue" then Orn (get gi1) (And (get gi2) (Wp ai2 (lut2!par2))) else	-- gi1 --> wp(ai1,T) /\ (gi1 \/ gi2) is implied by another path
								  Orn (And (get gi1) (Wp ai1 (lut2!par1))) (get gi2) 		-- gi2 --> wp(ai2,T) /\ (gi1 \/ gi2) is implied by another path
                           else		
