...
-- Calculate all possible (where the guard is not "false") subsequent actions of a workflow (only used in Precond3.hs and Precond5.hs)
subseqActionsSeq :: SeqWf -> Set GuardedAction
subseqActionsSeq Skip = Data.Set.empty
subseqActionsSeq (Choice (Normal (Atom "false"),a1) wf1
                         (Normal (Atom "false"),a2) wf2) = Data.Set.empty
subseqActionsSeq (Choice (Normal (Atom "false"),a1) wf1
                          ga2 wf2) = Data.Set.insert ga2 (subseqActionsSeq wf2)
subseqActionsSeq (Choice ga1 wf1
                         (Normal (Atom "false"),a2) wf2) = Data.Set.insert ga1 (subseqActionsSeq wf1)
subseqActionsSeq (Choice ga1 wf1
                         ga2 wf2) = Data.Set.union (Data.Set.insert ga1 (subseqActionsSeq wf1))
                                                   (Data.Set.insert ga2 (subseqActionsSeq wf2))
...
subseqActionsPar :: ParWf -> Set GuardedAction
...
indepPair :: (GuardedAction,GuardedAction) -> Set GuardedAction -> Bool
indepPair (ga1,ga2) gas = Data.Set.foldl (\c gb -> c && 
                                                  (indepActions ga1 gb) &&
                                                  (indepActions ga2 gb)) (True) gas

-- calculate the first index of an guarded-action-pair that is (not delayed and) independent from all guarded-action-pairs from the other workflows 
...
-- It is assumed that independence of guarded actions (g,a), (h,b) can be decided in the
-- following sense: indepActions (g,a) (h,b) expresses that g&a ; h&b \subseteq h&b ; g&a and
-- that a, b is deterministic; this should be equivalent to 
-- \forall I. g /\ wp(a,h /\ wp(b,I)) -->  h /\ wp(b,g /\ wp(a,I)) and a, b is deterministic
indepActions :: GuardedAction -> GuardedAction -> Bool
indepActions gb (Delayed g,a) = indepActions gb (Normal g,a)
indepActions (Normal (Atom "false"),_) _ = True
indepActions _ (Normal (Atom "false"),_) = True
