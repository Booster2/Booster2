...
lemma t3: "\<lbrakk> det a1 ; det a2 ;
             ...
             ||\<longrightarrow>||
              ...
              (h2 |\<longrightarrow>| wp(b2, T))
               |\<and>|
              (h1 |\<or>| h2)"
  apply(rule m_imp_trans)
  apply(rule t2, simp, simp, simp)

  apply(rule m_imp_imp_conj)
  apply(rule m_imp_and1)
  apply(rule f_imp_m_imp_cong)
  apply(rule m_imp_refl)
  apply(rule wp_m_imp_T)

  apply(rule m_imp_imp_conj)
  apply(rule m_imp_and2)
  apply(rule m_imp_and1)
  apply(rule f_imp_m_imp_cong)
  apply(rule m_imp_refl)
  apply(rule wp_m_imp_T)
